...
//
// - Debug builds install as: agentry-cli-debug
// - Release builds install as: agentry-cli
//
...
enum CLIPathInstaller {
    ...
    #if DEBUG
        nonisolated static let identity = MCPFilesystemIdentity.agentry(.debug)
    #else
        nonisolated static let identity = MCPFilesystemIdentity.agentry(.release)
    #endif
    ...
    static var bundledCLIPath: String? {
        Bundle.main.url(forAuxiliaryExecutable: "agentry-mcp")?.path
    }

    /// Built-in Claude Code tools to disable in the claude-agentry wrapper.
    /// These overlap with Agentry MCP tools and should use Agentry versions instead.
    ...
    /// Path to the stable MCP config file used by the claude-agentry wrapper.
    ...
    private static func claudeRPScriptContent() -> String {
        ...
        # Generated by Agentry
        #
        # This wrapper runs Claude Code with:
        #   - Agentry MCP server enabled via config file
        ...
        #   - Built-in file tools disabled (uses Agentry's MCP tools instead)
        #
        # Prerequisites:
        #   1. Claude Code CLI installed (https://docs.anthropic.com/claude-code)
        #   2. Agentry app running with MCP server enabled
        #
        ...
        if [[ -z "$claude_bin" ]]; then
            echo "\(claudeRPCommandName): Claude Code CLI not found. Install Claude Code, add claude to PATH, or ensure ~/.claude/local/claude is executable." >&2
            exit 127
        fi
    ...
    static func statusDescription() -> String {
        ...
        case .installedByOther:
            "Path exists but not managed by Agentry"
    ...
    enum InstallError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case let .pathExistsNotOurs(path):
                "A file already exists at \(path) that wasn't created by Agentry"
    ...
    // MARK: - claude-agentry Wrapper Installation

    /// Installation status specific to the claude-agentry wrapper
    ...
    /// Check the claude-agentry wrapper installation status
    @MainActor
    ...
    /// Human-readable description of claude-agentry status
    @MainActor
    ...
    /// Install the claude-agentry wrapper script using AppleScript for admin privileges
    @MainActor
    static func installClaudeRP() async throws {
        ...
        case .installed:
            ...
            logger.info("claude-agentry already installed at \(claudeRPInstallPath)")
        ...
        let tempFile = tempDir.appendingPathComponent("claude-agentry-\(UUID().uuidString).sh")
        ...
        logger.info("Installing claude-agentry: \(shellCommand)")
        ...
        do {
            ...
            logger.info("claude-agentry installed successfully at \(claudeRPInstallPath)")

            // Create the MCP config file that claude-agentry references
            try await MCPConfigExportService.shared.prepareStableWrapperConfigFile()
            logger.info("MCP config file created at \(claudeRPConfigPath)")
        } catch {
            logger.error("Failed to install claude-agentry: \(error.localizedDescription)")
            if (error as NSError).domain == "com.apple.osascript" {
                throw InstallError.permissionDenied
            }
            throw InstallError.unknown(error)
    ...
    /// Uninstall the claude-agentry wrapper from PATH
    @MainActor
    static func uninstallClaudeRP() async throws {
        ...
        switch status {
        case .notInstalled, .directoryMissing:
            logger.info("claude-agentry not installed, nothing to uninstall")
            return
        case .installedByOther:
            throw InstallError.pathExistsNotOurs(path: claudeRPInstallPath)
        ...
        logger.info("Uninstalling claude-agentry: \(shellCommand)")
        ...
        do {
            ...
            logger.info("claude-agentry uninstalled successfully")
        } catch {
            logger.error("Failed to uninstall claude-agentry: \(error.localizedDescription)")
            if (error as NSError).domain == "com.apple.osascript" {
                throw InstallError.permissionDenied
            }
            throw InstallError.unknown(error)
    ...
    private static func wrapperOwnershipFunction() -> String {
        let marker = shellQuoted(ManagedCLIPathPolicy.currentClaudeWrapperMarker)
        let check = "head -n 8 \"$candidate\" | grep -Fqx \(marker)"
        return "managed_wrapper(){ candidate=\"$1\"; if [ -e \"$candidate\" ] || [ -L \"$candidate\" ]; then [ -f \"$candidate\" ] && [ ! -L \"$candidate\" ] || return 1; \(check) || return 1; fi; }"
    }
