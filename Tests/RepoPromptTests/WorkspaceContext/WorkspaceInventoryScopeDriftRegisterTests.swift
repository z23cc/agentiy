...
// | D-5   | VERIFIED after P5-5                                     | `InventoryScopeConfig.self_check_patches` is DEBUG-default. Every eligible Rust patch is canonical-byte-compared outside the state lock against an authoritative rebuild; match publishes the patch, mismatch records `.shadowValidationMismatch` and atomically publishes the authoritative generation. `inventory_scope_contract::{self_check_matching_patch_remains_patched_and_records_diagnostics,shadow_validation_mismatch_fails_closed_to_authoritative_generation,stale_self_check_attempt_uses_backstop_without_recording_a_false_comparison}` pins both arms plus stale-base exclusion; `inventory_scope_events::shadow_validation_mismatch_precedes_authoritative_generation_events` pins event order. Existing public diagnostic fields now expose Rust counters rather than zero-valued tombstones. |
