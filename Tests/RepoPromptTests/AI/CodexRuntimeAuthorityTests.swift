...
final class CodexRuntimeAuthorityTests: XCTestCase {
    ...
    func testBundledRuntimeResolvesRequestedArchitectureAndOwnedState() throws {
        ...
        XCTAssertEqual(runtime.version, .init(major: 0, minor: 151, patch: 0))
        ...
        XCTAssertTrue(runtime.redactedDiagnosticSummary.contains("version=0.151.0"))
    ...
    func testVersionParserRejectsMalformedTokensAndInvalidNumericPrereleaseIdentifiers() {
        XCTAssertNil(CodexRuntimeAuthority.Version.parse("codex-cli 0.151.0.1"))
        XCTAssertNil(CodexRuntimeAuthority.Version.parse("codex-cli 0.151.0-rc.01"))
        XCTAssertEqual(
            CodexRuntimeAuthority.Version.parse("codex-cli 0.151.0-rc.1"),
            .init(major: 0, minor: 151, patch: 0, prerelease: "rc.1")
        )
    }
    ...
    func testExplicitExternalOverrideIsAbsoluteVersionGatedAndObservable() async throws {
        ...
        let accepted = try CodexRuntimeAuthority.resolve(
            ...
            externalVersionReader: { _ in "codex-cli 0.151.0" }
        ).get()
        XCTAssertEqual(accepted.source, .externalOverride)
        XCTAssertEqual(accepted.version, .init(major: 0, minor: 151, patch: 0))
        ...
        XCTAssertEqual(
            ...
            .externalOverrideTooOld(
                ...
                minimum: .init(major: 0, minor: 151, patch: 0)
            )
        )
        ...
        let prerelease = CodexRuntimeAuthority.resolve(
            ...
            applicationSupportURL: temporaryDirectory,
            explicitExecutableOverride: override.path,
            externalVersionReader: { _ in "codex-cli 0.151.0-rc.1" }
        )
        XCTAssertEqual(
            failure(from: prerelease),
            .externalOverrideTooOld(
                actual: .init(major: 0, minor: 151, patch: 0, prerelease: "rc.1"),
                minimum: .init(major: 0, minor: 151, patch: 0)
            )
        )
    ...
    func testOverrideEnvironmentIsTheOnlyFallbackWhenBundleIsMissing() throws {
        ...
        let runtime = try CodexRuntimeAuthority.resolve(
            ...
            resourcesURL: nil,
            applicationSupportURL: temporaryDirectory,
            externalVersionReader: { _ in "codex 0.151.0" }
        ).get()
    ...
    func testCodexPreflightUsesCapturedLoginShellOverrideInsteadOfInheritedAppEnvironment() async throws {
        ...
        XCTAssertEqual(resolution.runtime?.version, .init(major: 0, minor: 151, patch: 0))
        XCTAssertEqual(resolution.displayDescription, "External Codex override 0.151.0 (codex)")
