...
| wildmatch / OpenBSD-derived fnmatch material | `Sources/RepoPromptDomainRuntime/PathMatching/RepoWildmatch.swift` | The active close Swift translation preserves the copyright and BSD-style license notice from the former `wildmatch.c`. | The full notice text retained from the former `wildmatch.c` and `wildmatch.h`, including the header's advertising acknowledgement condition, remains in root `THIRD_PARTY_NOTICES.md` and is bundled under `Contents/Resources/Legal` during app packaging. |
