...
| **G8 Governance** | conductor-only heavy entry, CI, guardrails, preflight and dependency/license policy | conductor/CI/preflight evidence | first-class Cargo operations claim `build` + heavy admission; Make aliases and archive-before-Swift verification are active; `.github/workflows/ci.yml` has an arm64 Rust FFI job; Rust-specific PR-ready selection and mandatory deny/audit/fuzz CI execution remain open |
