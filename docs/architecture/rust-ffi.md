...
| **G4 Subscription/Wake** | 3 queue/wake/lifecycle; 4 representative payload benchmark | **Conditional pass** | `subscription_backpressure.rs`; `wake_pipe.rs`; `shutdown_races.rs`; focused ThreadSanitizer `AgentryCoreBridgeTests`; synthetic fixtures in `rust/benchmarks/fixtures/v1/synthetic/`; caps in `rust/benchmarks/slo-v1.json` | Queue, wake, shutdown, and synthetic shapes are verified. A redacted real current-Swift baseline and measured comparison against the frozen SLOs do not exist. This is a **P1 prerequisite** and must not be replaced by synthetic-only results. |
...
UniFFI is the **accepted** raw binder (ADR-0001, user decision 2026-08-20), within the capability envelope proven by the gates. G4's real-baseline/SLO comparison remains a P1 prerequisite; the explicitly listed G7 evidence-registration gap remains a registered follow-up.
