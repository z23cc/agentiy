...
| `NativeAgentRuntimeControlling` conformance | Swift adapter | `ClaudeRustBackedNativeSessionAdapter`; sole construction site is `ClaudeAgentModeCoordinator.makeDefaultController` |
...
### 15.6 D-9/R9 raw-event observability parity, implemented; real-traffic certification remains P6-8

The Rust-backed arm now writes the Claude DEBUG raw-event JSONL itself; no raw line or decoded payload crosses the FFI merely for logging. Swift resolves the existing `claudeRawEventLoggingEnabled` and `claudeRawEventLogFilePath` settings through the same `ClaudeNativeProcessSessionController` helpers as the legacy arm and passes only the resolved enabled flag, absolute path, and non-secret record context into `CoreAgentClaudeScopeConfigV1`. This preserves §11.7's one-switch rule and R8: process environment and other secret-bearing inputs are never logged.

**Envelope and concurrency.** `raw_event_log::RawEventLogWriter` serializes record construction and append across stdout, stderr, and command workers. Every record carries `kind`, UTC `timestamp`, `runID`, `tabID`, `windowID`, and `sessionID`; `session.header` precedes the first record for each observed session ID and also carries `workspacePath`. The record context is threaded from `ClaudeRustBackedNativeSessionAdapter` through `CoreAgentSession`/UniFFI. Raw bytes use the same 64 KiB UTF-8-or-base64 sampling contract as Swift. Session-ID changes are generation-serialized with writes, so a new header cannot be interleaved behind a record carrying that new ID.

**Payload parity.** The port does not stop at kind strings. Startup carries `existingSessionID`/`model`/`effortLevel`/`hasSystemPromptOverride`; spawn omits PID/environment exactly as Swift does; control request/response records retain the full request/response/error/count shapes; permission-mode initialization retains requested mode plus response; interrupt records retain reason/error; decoded stream/control payloads are retained; translator records preserve the legacy 11 fixed camel-case keys (including JSON null values) while `toolArgsJSON`/`toolResultJSON` retain their legacy conditional presence; `system/init` reconstructs tools and MCP-server statuses; recovery records retain the same line samples/previews and aggregate counts. The startup API therefore gained optional model/effort inputs solely to preserve the pre-resolution observability payload; these values do not alter Rust spawn or flag-setting authority.

**Frozen 44-kind accounting.** A source scan finds 43 of §10's 44 literals in `rust/crates/runtime/src/agent_claude/`. They map as follows:

| Group | Frozen kinds | Rust owner |
|---|---|---|
| Session/process | `session.startOrResume`, `process.spawned`, `process.stderr`, `process.stdoutEOF`, `session.initialized`, `session.permissionModeInitialized`, `session.flagSettingsDeferred`, `session.flagSettingsPending`, `session.flagSettingsApplied`, `session.shutdown`, `session.failProtocol` | `scope.rs`, `raw_event_log.rs` |
| Raw/decode/recovery/framing | both `protocol.*.raw`; all stream/control/keep-alive/recovered inbound kinds; `protocol.decode.skipped`, concatenated/recovered/control-char/plaintext kinds; both `framer.*` kinds | `scope.rs`, `recovery.rs`, `framer.rs` |
| Translation/lifecycle | `runtime.init.stream`, both `translator.streamResult*`, `lifecycle.idleFallback`, all three interrupt terminal kinds | `scope.rs`, `turn_state.rs` |
| Control/approval | all four `control.*` kinds; `approval.request.emitted`, both auto-approve kinds, `approval.response.sent` | `scope.rs`, `permission.rs`, `control.rs` |
| Named replacement | `protocol.decode.failed` | No honest Rust emission site; see below. |

The sole absent literal, **`protocol.decode.failed`**, is not silently dropped. Swift emits it only from a generic `catch` after an exhaustive `catch CodecError`; Rust's decoder is a total `Result<Option<InboundMessage>, CodecError>` and has no third exception class. Invalid JSON remains `protocol.decode.skipped`, unsupported typed payloads increment/publish `protocolDrift`, and an actual Rust panic is captured by the process-wide panic hook with file/line/backtrace. Fabricating an unreachable `protocol.decode.failed` record would weaken, not improve, the contract. This is the named reviewed replacement permitted by §10.

Three other limits are explicit rather than hidden:

1. `approval.autoApprove.fallback` exists but is unreachable while both arms' approval builders are total; its literal and explicit Rust decision variant remain so a future reachable fallback cannot disappear silently.
2. `protocol.decode.recoveredJSONStringControlChars` is retained and unit-driven, although §6's proof shows the codec's own sanitize pass consumes every reachable successful case first.
3. Rust keeps writing to the initial Swift-resolved file path after a real session ID arrives; headers and records switch to the real ID, but the filename is not rotated. Reproducing Swift's timestamped file rotation inside Rust would duplicate a host file-policy algorithm, while crossing back into Swift from the reader hot path would violate the boundary. Consumers must key records by envelope `sessionID`, not filename. This is an observability-file-layout difference, not lost content.

`toolInvocationID` also preserves JSON string shape but not cross-arm value identity: Rust's translator uses its documented monotonic invocation ID while Swift uses a UUID. Differentials normalize arm-local IDs by first appearance and still assert call/result correlation.

**Automated gates.** `raw_event_log_completeness_a_representative_session_produces_every_naturally_reachable_kind` drives the real synthetic child through handshake, malformed-line recovery, system init, translation, approval, result, flag settings, and shutdown, then asserts both kind presence and load-bearing payload fields. Mapping tests cover non-coincident recovery/framer branches; directed tests cover interrupt, pending/restart settings, idle fallback, and session-header transitions. Cargo runtime/FFI tests, deterministic UniFFI codegen, and the two-arm Swift differential are the focused P6-7 gate. The bounded real-Claude >=20-session corpus/soak remains P6-8 because it requires a visible debug-app lifecycle action and real API spend; it is not claimed here without explicit user approval.

### 15.7 Swift bridge wake-drain ordering, root cause closed

D-9/R9's additional raw-log work changed timing enough for the turn-level differential to expose a second inversion: Rust published `toolResult` before the following `approvalRequest`, but Swift occasionally yielded the approval first. The Rust path was ruled out structurally and by `tool_result_and_following_approval_request_drain_in_stdout_order`: the single stdout reader calls `handle_line` sequentially, both events are lossless, `SubscriptionHub` appends them to one `VecDeque`, and `try_drain` pops from its front.

The defect was in `AgentryCoreBridge.wakeFired()`. Each `DispatchSourceRead` callback started a new task. Although the bridge is an actor, `drain(subscriptionID:)` awaits the asynchronous `CoreEventDecoding.decode` for every event; actor reentrancy therefore allowed a second wake task to remove and yield a newer Rust batch while the first task was suspended decoding an older batch. Actor isolation serialized synchronous regions, not the whole logical drain transaction.

**Fix:** wake handling is now single-flight. `wakeDrainInProgress` grants one logical drainer ownership; overlapping callbacks set `wakeDrainRequested` and return. The owner completes its rearm/drain cycle and repeats while that pending bit is set before relinquishing ownership. No sleep, priority rule, or event-type special case participates in ordering. `CoreSubscriptionTests.testOverlappingWakeCallbacksCannotReorderEventsAcrossAnAsyncDecode` gates the first decode, injects a newer terminal batch plus repeated wakes, proves only one transport drain occurs while gated, then asserts payload and authority-sequence order. The Rust scope order test and the real two-arm `ClaudeRustBackedTurnLevelDifferentialTests` provide the adjacent lower and upper integration gates.

### 15.8 P6-8 real-CLI soak and production cutover

**Authority transfer (2026-08-24).** `ClaudeAgentModeCoordinator.makeDefaultController` now constructs `ClaudeRustBackedNativeSessionAdapter` directly for `.standard` in debug and release builds. GLM, Kimi, and custom-compatible variants retain `ClaudeCompatibleNativeSessionAdapter`/`ClaudeNativeProcessSessionController` until P6-9. `ClaudeRustBackedNativeSessionAdapterSelection.swift`, `AGENTRY_CLAUDE_RUST_BACKED_ADAPTER`, and the adapter's `#if DEBUG` wrapper are deleted. Standard Claude has no live fallback; rollback is source/artifact rollback only, as required by §14.

**Bounded real-CLI evidence.** The approved visible-app/API-spend soak exercised more than twenty real Claude Code sessions, including repeated multi-turn resume, at least three real `can_use_tool` approval round trips, three interrupt-and-recover runs, a Haiku-to-Sonnet model/effort change, forced supervised-process death, and app termination during an active turn. The process-death case exposed one real blocker before cutover: stdout EOF flushed the turn but left `AgentClaudeScope.state.process` populated, so the Swift adapter reused a dead stdin and the next turn failed with `Broken pipe`. The fix adds a lossless terminal `processExited` fact, fences both process and stdin cleanup by PID so late EOF cannot clear a replacement child's stdin, invalidates the Swift adapter's initialized state, explicitly closes the old bridge subscription/scope, and preserves the outer event stream plus provider session ID while a replacement scope starts. The synthetic restart test and focused Swift differential gate this path; the real soak then killed the Claude process group, observed the active turn fail, and completed the next steer through `--resume` with the same provider session ID. App termination likewise left no Agentry or Claude child process, and a freshly launched app completed a new real session.

**Steering cutover detail.** `interruptClaudeTurnIfNeeded` no longer trusts an event-derived `hasTurnInFlight` precheck. It always observes the MCP/tool-ack safe point and issues the idempotent interrupt command; `.noTurnInFlight` is success, while timeout/failure retains the post-command liveness recheck. This removes the stale-read race without moving the host-owned MCP safe point into Rust.

**Cutover review hardening.** Four cross-boundary failures found at the final review gate are covered explicitly. Stdout EOF clears stdin only when the EOF PID still owns that slot, preventing a late old-reader EOF from poisoning a replacement process. Correlated interrupt outcomes that beat Swift continuation registration are retained by request ID and consumed on registration instead of being dropped into the outer timeout. A user-selected permission decision whose transport write fails now emits the terminal protocol error and closes the unusable core scope, matching the legacy controller rather than swallowing the failure. Finally, `startOrResume` returns the supervised child PID; the Swift façade registers that exact PID with the expected-agent MCP fence before exposing the session and clears the same PID on process exit/replacement/deinit. The restart differential asserts register-then-clear pairs across two process generations. Focused Rust ownership, early-interrupt, failed-permission-response, and expected-PID regressions gate these paths.

**Real sleep/wake certification tail, explicitly not claimed.** P6-4's fake monotonic-clock discontinuity matrix is green, but this machine could not safely run unattended physical sleep: macOS documents `IOPMSchedulePowerEvent` as root-only and passwordless elevation is unavailable, so there was no way to guarantee an automatic wake. With explicit user approval, the cutover proceeds while real physical sleep/wake remains a named external certification item; no test result is fabricated and §11's parity semantics remain unchanged.

### 15.9 P6-9 compatible-variant authority transfer

**One authority for four variants (2026-08-24).** `ClaudeAgentModeCoordinator.makeDefaultController` now constructs `ClaudeRustBackedNativeSessionAdapter` unconditionally for standard Claude Code, GLM, Kimi, and custom-compatible interactive sessions. There is no variant branch, DEBUG selector, environment rollback flag, `ClaudeCompatibleNativeSessionAdapter`, or `ClaudeNativeProcessSessionController` construction in the production factory. The legacy Swift controller remains only as a differential oracle until the P6-10 caller/deletion audit; rollback remains source/artifact rollback.

**Host facts stay host-owned.** Swift continues to resolve the executable path, backend-specific credentials and launch environment, permission mode, MCP config, and GLM's exact `--append-system-prompt` input. Rust receives those resolved launch facts and remains the sole process/framing/codec/translation/turn/control authority. The same expected-agent PID fence from §15.8 applies to every variant because all four provider kinds require PID-owned Agent Mode MCP routing.

**Variant differential.** `agent-claude-synthetic-cli` can record its original production argv plus only an explicit allowlist of environment keys; it never dumps the ambient environment. `ClaudeRustBackedTurnLevelDifferentialTests.testCompatibleVariantsPreserveLegacyLaunchAndTurnBehaviorAcrossBothArms` drives GLM, Kimi, and custom-compatible through both the frozen Swift oracle and Rust adapter using the same real child process. It asserts identical canonical turn events, normalized argv (only the arm-local MCP-config path is normalized), and the selected environment facts. The GLM arm additionally pins the literal `Running within this desktop app.` append prompt and verifies the other variants do not receive that flag.

**Certification boundary.** The current debug app has no configured Z.AI, Moonshot, or custom-compatible credentials, so this cutover does not claim paid real-backend traffic for those three variants. The exact launch/turn differential, production-topology guard, focused Rust runtime suite, deterministic UniFFI regeneration, and focused Swift cutover suite are the acceptance evidence. Real external-backend smoke remains a named credential-dependent certification tail rather than a fabricated gate result.

### 15.10 P6-10 obsolete Swift runtime retirement

**Deletion audit (2026-08-24).** Production and test callers were re-audited after P6-9. The obsolete `ClaudeNativeProcessSessionController`, `ClaudeCompatibleNativeSessionAdapter`, core Swift codec/translator façades, codec shadow comparator, DEBUG selector, and their controller-specific regression tests had no live caller and were deleted. The temporary raw `agent_claude_decode_line_debug_v1` bridge plus its Rust `debug_shadow` modules were also removed; no per-line Claude export remains in any profile. Source-layout and Rust FFI guardrails now reject reintroduction and pin the runtime crate back to its two required unsafe-code exception sites.
