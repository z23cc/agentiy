...
**Supply-chain note (dependency-surface delta, recorded for `cargo deny`; see §12).** `nix = "=0.30.1"` is already a pinned `rust/` workspace dependency (`rust/Cargo.toml:31`), already consumed by `wake_pipe.rs` for pipe/fcntl work; `libc 0.2.189` is already present transitively (`rust/Cargo.lock`). The reaper/spawner need feature flags on this already-audited crate (kqueue/event, process, signal), not a new third-party crate. **`[workspace.lints.rust] unsafe_code = "forbid"` is set at `rust/Cargo.toml:53`.** `posix_spawn`'s `POSIX_SPAWN_CLOEXEC_DEFAULT`/`posix_spawn_file_actions_addchdir_np` and the raw kqueue `EVFILT_PROC`/`EVFILT_TIMER` registrations both require FFI calls that are `unsafe` in Rust. **This is a P6-4 prerequisite this document surfaces but does not resolve**: the crate that hosts the spawner/reaper (expected: `agentry-runtime`, or a narrower sub-module) needs an explicit, narrowly-scoped `#[allow(unsafe_code)]` exception — a policy change, not merely a feature-flag change — and P6-4 must land it deliberately rather than discover it mid-implementation. If the `nix` feature pinning surfaces an unexpected gap, the design's stated fallback is hand-declared `extern "C"` bindings for the syscalls involved (`posix_spawn`, `posix_spawn_file_actions_*`, `posix_spawnattr_*`, `kqueue`, `kevent`, `waitid`, `waitpid`) — which needs *more* `unsafe`, not less, so the exception is unavoidable either way.
...
P6-1 pins the exact feature/extern set §5.2/§5.3 will need at P6-4 and re-runs `cargo deny` against it now, before any implementation exists, so the supply-chain decision is reviewed at contract-freeze time rather than discovered mid-slice. See the commit landing this pinning for the recorded `cargo deny` result and whether `rust/Cargo.lock` changed (an unchanged lock is itself the evidence the delta is feature-flags-only, not a new dependency). The `unsafe_code = "forbid"` workspace lint and its P6-4 exception requirement (§5.2) are recorded here as a named, un-silently-discovered prerequisite, not resolved by this document.
