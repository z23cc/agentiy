...
Canonical MCP schemas and tool-catalog policy are authored once in `rust/crates/proto/catalog/mcp_catalog_v1.json`. Rust exposes the exact canonical payload plus typed records; `AgentryCoreBridge` verifies the exact canonical bytes, digest, and shape, and `MCPDomainRuntime` validates and atomically installs one immutable `MCPDomainCatalogSnapshot` into both `MCPDomainToolRegistry` and `MCPDomainHost` before bootstrap. Registration verifies canonical binding fingerprints; advertisement, capability classification, operation policy, and application/window/repository resource admission all consume that installed snapshot. P11 also routes final operation validation, admission-class evidence, and registration-scope lookup through the host snapshot and its injected Rust resolver; pending invocation IDs are reserved before asynchronous handoff so duplicate calls and drain accounting remain deterministic. The generated `MCPDomainGeneratedToolDefinitions` projection remains only a compatibility fixture for direct unit-test construction, never a production handoff. There is no legacy service registry, copied resource manifest, live-window recorder, or mixed catalog authority. Provider-built app tools already contain one `MCPAppToolBinder` execution envelope and are projected without wrapping them again; raw shared bindings receive exactly one envelope. Invalid, duplicate, noncanonical, or limit-inconsistent catalogs fail by throwing during startup registration rather than trapping.
...
- `Scripts/m8_live_certification.sh` records the operational provider, backend, power, and signed-artifact gates against the current commit. `Scripts/validate_m8_live_evidence.py` is the fail-closed receipt authority: it rejects secret/path material, stale heads, schema drift, incomplete matrix evidence, and unauthorized `app` → `auto` promotion. Missing credentials or host/release prerequisites remain blocked/deferred; M8 never infers live success from offline tests.
