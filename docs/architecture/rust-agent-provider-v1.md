...
Status: P7-2 whole-stage implementation baseline (Codex and ACP semantic authority; Claude headless remains on the P6 translator contract)
...
small lifecycle state needed by controllers. Claude headless remains translator-
...
request/cancel/notify/respond/respond-error/state operations. Response IDs cross
...
`CodexAppServerRuntimeSession` or `AcpRuntimeSession` capability. Controllers use
that capability for request, cancellation, notification, and server-request
response paths; the old Swift pending/correlation/parser path remains only for
explicitly injected legacy test clients.
...
- Runtime, FFI, bridge, and provider tests cover identity, request settlement,
  ...
  server-request events, lifecycle state, malformed input, ID-type separation,
