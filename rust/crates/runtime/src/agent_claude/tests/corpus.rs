...
fn one_line(name: &str) -> Vec<u8> {
    ...
    assert_eq!(
        lines.len(),
        1,
        "{name}: expected exactly one framed line, got {}",
        lines.len()
    );
...
fn concatenated_split_positive_recovers_both_segments() {
    ...
    assert_eq!(
        ...
        "trailing data after the first close-brace must fail primary decode"
    );
    ...
    match outcome {
        RecoveryOutcome::Recovered(messages) => {
            assert_eq!(messages.len(), 2, "both segments must decode independently")
        }
        other => panic!("expected Recovered(2 messages), got {other:?}"),
    }
    assert!(
        diagnostics.contains(&RecoveryDiagnostic::Recovered {
            segments: 2,
...
fn concatenated_split_negative_finds_zero_segments_and_every_heuristic_declines() {
    ...
    assert_eq!(
        ...
        "unterminated JSON candidate: no heuristic can recover it, even with a turn in flight"
    );
}
...
fn embedded_tail_positive_recovers_the_trailing_object() {
    ...
    let expected_start = line.len()
        - br#"{"type":"system","subtype":"status","status":"recovered-from-tail"}"#.len();
    assert!(
...
fn embedded_tail_negative_finds_zero_marker_occurrences() {
    ...
    assert_eq!(codec::decode_line(&line), Err(CodecError::InvalidJson));
    assert_eq!(
        recovery::recover_invalid_json_line(&line, true, |_| {}),
        RecoveryOutcome::Declined
    );
}
...
fn json_control_char_repair_fixture_is_handled_by_the_codecs_own_sanitize_not_the_controller_heuristic()
 {
    ...
    let decoded = codec::decode_line(&line)
        .expect("codec's own inline sanitize must succeed here")
        .unwrap();
    match decoded {
...
fn plaintext_salvage_positive_only_fires_while_a_turn_is_in_flight() {
    ...
    assert_eq!(
        outcome,
        RecoveryOutcome::PlaintextSalvage(expected_text.to_string())
    );
...
fn crlf_line_ending_decodes_cleanly_after_framer_strips_cr_and_lf() {
    ...
    assert!(matches!(
        decoded,
        crate::agent_claude::InboundMessage::StreamPayload(_)
    ));
}
...
fn realcapture_thinking_and_reasoning_dropped() {
    ...
    assert_eq!(
        assistant_results[0].text.as_deref(),
...
fn realcapture_session_state_changed_idle_sequence() {
    let lines: Vec<Vec<u8>> =
        read_fixture("realcapture-session-state-changed-idle-sequence.ndjson")
            .split(|&b| b == b'\n')
            .filter(|l| !l.is_empty())
            .map(<[u8]>::to_vec)
            .collect();
...
fn realcapture_can_use_tool_round_trip_decodes_at_codec_level() {
    ...
    assert_eq!(
        request.request.get("tool_name").and_then(|v| v.as_str()),
        Some("Bash")
    );
