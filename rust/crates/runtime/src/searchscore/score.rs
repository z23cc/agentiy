...
fn wildmatch_from(
    ...
    let result = if pattern_index == pattern.len() {
        text_index == text.len() || text.get(text_index) == Some(&b'/')
    } else {
        match pattern[pattern_index] {
            ...
            b'\\' => {
                let Some(literal) = pattern.get(pattern_index + 1) else {
                    return false;
                };
                text.get(text_index)
                    .is_some_and(|byte| byte_eq(*literal, *byte))
                    && wildmatch_from(pattern, text, pattern_index + 2, text_index + 1, memo)
            }
            literal => {
                text.get(text_index)
                    .is_some_and(|byte| byte_eq(literal, *byte))
...
fn character_class(pattern: &[u8], mut index: usize, value: Option<u8>) -> Option<(bool, usize)> {
    ...
    while index < pattern.len() && pattern[index] != b']' {
            ...
            && pattern.get(index + 1).is_some_and(|byte| *byte != b']')
        {
            ...
            matched |= (lower..=upper).contains(&value);
