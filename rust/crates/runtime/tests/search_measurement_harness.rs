...
use agentry_runtime::{
    CompactRegexBatchResult, JitStatus, LeafCancellation, MatchPolicy, RegexSearchMode,
...
fn compact_output(result: &CompactRegexBatchResult) -> Value {
    ...
    json!({
        ...
        "lineRangeWordCount": result.line_range_words.len(),
        "materializedUTF8Bytes": 0
    })
}

fn fixture_subjects(fixture: &str, bytes: &[u8]) -> Vec<String> {
...
fn measure_rust_search_core_floor_v1() {
    let output = PathBuf::from(std::env::var("AGENTRY_RUST_SEARCH_CORE_FLOOR_OUTPUT").unwrap());
    let fixture_path =
        PathBuf::from(std::env::var("AGENTRY_RUST_SEARCH_CORE_FLOOR_FIXTURE_PATH").unwrap());
    let fixture = std::env::var("AGENTRY_RUST_SEARCH_CORE_FLOOR_FIXTURE").unwrap();
    let warmups = std::env::var("AGENTRY_RUST_SEARCH_CORE_FLOOR_WARMUPS")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(100_usize);
    let samples = std::env::var("AGENTRY_RUST_SEARCH_CORE_FLOOR_SAMPLES")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(1_000_usize);

    let fixture_bytes = fs::read(fixture_path).unwrap();
    let subjects = fixture_subjects(&fixture, &fixture_bytes);
    let requests: Vec<_> = subjects
        .iter()
        .map(|subject| RegexSearchRequest {
    ...
    for iteration in 0..(warmups + samples) {
        ...
        jit_active &= result
            .subject_summaries
            .iter()
            .all(|value| value.diagnostic.jit_status == JitStatus::Active);
        cache_hits += result
            .subject_summaries
            .iter()
            .filter(|value| value.diagnostic.cache_hit)
            .count() as u64;
