...
fn every_named_agent_claude_export_is_present_in_the_reviewable_inventory() {
    for expected in EXPECTED_AGENT_CLAUDE_EXPORTS {
        assert!(
            ...
            "exports.txt is missing or has drifted from the frozen signature: {expected}"
        );
    }
}
...
fn no_agent_claude_export_accepts_a_single_protocol_line() {
    ...
    let agent_claude_lines: Vec<&str> = EXPORTS
        .lines()
        .filter(|line| line.contains("CoreRuntime.agent"))
        .collect();
    assert_eq!(
        agent_claude_lines.len(),
        EXPECTED_AGENT_CLAUDE_EXPORTS.len(),
        "unexpected agent-claude export count in exports.txt: {agent_claude_lines:?}"
    );
    for line in agent_claude_lines {
        assert!(
            !line.contains("bytes"),
            "an agent-claude export takes a raw `bytes` parameter, which INV-P6-1 forbids for production exports: {line}"
        );
    }
}
